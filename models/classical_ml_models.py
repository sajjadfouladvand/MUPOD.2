...
# validation_data=[]
# test_data=[]
# with open("training_data_aggregated.csv") as train_data_file, open("validation_data_aggregated.csv") as val_data_file, open("testing_data_aggregated.csv") as test_data_file:
#     train_header=next(train_data_file)
#     validation_header=next(val_data_file)
#     test_header=next(test_data_file)
#     for line in train_data_file:
#         line_splitted=line.split(',')    
#         line_splitted=[float(x) for x in line_splitted]
#         train_data.append(line_splitted)  
#     for line in val_data_file:
#         line_splitted=line.split(',')    
#         line_splitted=[float(x) for x in line_splitted]
#         validation_data.append(line_splitted)  
...
# val_data_ar = np.array(validation_data)            
# test_data_ar=np.array(test_data)
# #=========== Combining training and validation
# training_all=np.concatenate((train_data_ar, val_data_ar), axis=0)
# # training_all_labels=np.concatenate((trainingset_labels_ar, validationset_labels_ar), axis=0)
...
#                'min_samples_leaf': min_samples_leaf,
#                'bootstrap': bootstrap}
# with open('hyperparameters.csv', 'w') as csv_file:  
#     writer = csv.writer(csv_file)
#     for key, value in hyperparameters.items():
#        writer.writerow([key, value])
...
# randomCV.fit(training_all[:,1:-1], training_all[:,-1])

# best_rf_model= randomCV.best_estimator_
# feat_importances = pd.Series(randomCV.best_estimator_.feature_importances_, index=train_header.replace('\n','').replace('Normalized Age (at the beginig of study)','Age').split(',')[1:-1])

# fig = feat_importances.nlargest(len(train_header.split(',')[1:-1])).plot(kind='barh', grid=True,figsize=(12,10))
# fig.set_xlabel("Importance Score")
# fig.set_ylabel("Features")
# fig.get_figure().savefig("feature_importance.png", dpi=300)

# rf_predictions = best_rf_model.predict(test_data_ar[:,1:-1])

# rf_test_auc=roc_auc_score(test_data_ar[:,-1], best_rf_model.predict_proba(test_data_ar[:,1:-1])[:,1])
...
# specificity=0
# for asses_ind in range(len(rf_predictions)):
#     if(rf_predictions[asses_ind]==0 and test_data_ar[asses_ind,-1]==0):
#         tn=tn+1
#     elif(rf_predictions[asses_ind]==0 and test_data_ar[asses_ind,-1]==1):
#         fn=fn+1
#     elif(rf_predictions[asses_ind]==1 and test_data_ar[asses_ind,-1]==1):
#         tp=tp+1
#     elif(rf_predictions[asses_ind]==1 and test_data_ar[asses_ind,-1]==0):    
#         fp=fp+1
# accuracy=(tn+tp)/(tn+tp+fn+fp)
# if(tp+fp == 0):
#     precision=0
...
# with open("random_forest_results_balanced.csv", 'w') as f_results:
#     f_results.write("Precision is: ")
#     f_results.write(str(precision))
#     f_results.write("\n")
