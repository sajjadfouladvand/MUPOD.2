...
learning_rate = 0.01#random.choice(learning_rate_pool)
num_layers= 1#random.choice(num_layers_pool)
BATCH_SIZE=128#random.choice(BATCH_SIZE_pool)
dropout_rate=0.5#random.choice(dropout_rate_pool)
EPOCHS= 10#random.choice(EPOCHS_pool)
num_heads = 1#random.choice(num_heads_pool)
...
class Transformer(tf.keras.Model):
  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, 
    ...
    self.final_layer = tf.keras.layers.Dense(num_classes, kernel_regularizer='l2')#, activity_regularizer='l2')
...
validation_filename_meds_diags_demogs='outputs/validation_meds_diags_procs_demogs_represented_aug28.csv'
...
train_filename='outputs/train_meds_diags_procs_demogs_represented_aug28.csv'
...
header_results_filename= "Model Number (SINGLE STREAM), Learning Rate, Number of Layeres, Number of heads, Batch Size, Dropout Rate, Number of EPOCHS, Validation Accuracy, Validation Precision, Validation Recall, Validation F1-Score, Validation Specificity, Validation TP, Validation TN , Validation FP, Validation FN, Valication AUC, validation optimum threshold\n"
