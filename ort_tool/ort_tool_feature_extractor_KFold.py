...
# Reading the ORT tool ICD codes. I have created this table by looking into the ORT tool and the ICD dictunary we got from the HCUP website
...
for cv_idx in range(num_kfolds):
    ...
    current_fold_enrolids = pd.read_csv('outputs/test_demographics_shuffled_fold'+str(cv_idx)+'.csv')
    if cv_idx % 10==0:
        print(cv_idx)
    ...
    with open('outputs/test_medications_shuffled.csv') as test_meds_file, open('outputs/test_diagnoses_shuffled.csv') as test_diags_file, open('outputs/test_procedures_shuffled.csv') as test_procs_file:
        ...
        for line_diag in test_diags_file:
            ...
            if not(float(current_enrolid) in current_fold_enrolids['ENROLID'].values):
                line_counter-=1
                continue
            # pdb.set_trace()    
    ...
    test_set_ort_scores.to_csv('ort_tool/test_set_ort_scores_'+str(imb_ratio)+str(compute_for_imb)+'_KFold'+str(cv_idx)+'.csv', index=False)
    # pdb.set_trace()
...
results_all = pd.DataFrame(0, index=np.arange(num_kfolds), columns=['accuracy', 'precision', 'recall', 'F1_score', 'auc', 'tp','tn','fp', 'fn'])
results_all['accuracy'] = accuracy_all
results_all['precision'] = precision_all
results_all['recall'] = recall_all
results_all['F1_score'] = F1_score_all
results_all['auc'] = auc_all
results_all['tp'] = tp_all
results_all['tn'] = tn_all
results_all['fp'] = fp_all
results_all['fn'] = fn_all
results_all.to_csv('ort_tool/ort_tool_test_results'+str(imb_ratio)+str(compute_for_imb)+'_KFold.csv', index=False)
